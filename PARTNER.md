...
**Vor jedem Commit:** `git grep -n "http" -- data/partners.ts` prüfen — es dürfen ausschließlich `https://`-Deeplinks **zugelassener** Programme im Feld `affiliateUrl` erscheinen (Stand 09.07.2026: die sieben Awin-Links von Lexware Office, Wondershare PDFelement, it-versand.com, Tarifcheck.de, CHECK24 — Strom/Gas + DSL — und congstar), niemals `http://` und niemals URLs außerhalb von `affiliateUrl`.
