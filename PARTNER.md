...
**Vor jedem Commit:** `git grep -n "http" -- data/partners.ts` prüfen — es dürfen ausschließlich `https://`-Deeplinks **zugelassener** Programme im Feld `affiliateUrl` erscheinen (Stand 08.07.2026: die vier Awin-Links von Lexware Office, Wondershare PDFelement, it-versand.com und Tarifcheck.de), niemals `http://` und niemals URLs außerhalb von `affiliateUrl`.
