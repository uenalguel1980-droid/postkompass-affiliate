...
**Vor jedem Commit:** `git grep -n "http" -- data/partners.ts` prüfen — es dürfen ausschließlich `https://`-Deeplinks **zugelassener** Programme im Feld `affiliateUrl` erscheinen (Stand 08.07.2026: die sechs Awin-Links von Lexware Office, Wondershare PDFelement, it-versand.com, Tarifcheck.de und CHECK24 — Strom/Gas + DSL), niemals `http://` und niemals URLs außerhalb von `affiliateUrl`.
